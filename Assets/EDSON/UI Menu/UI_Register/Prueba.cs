...
{
    private const string ApiUrl = "http://localhost:3000/player"; 
    public UIDocument loginUI;  // Referencia a la pantalla de login
    private MenuManager menuManager; // Referencia a MenuManager
    ...
    void Start()
    {
        ...
        menuManager = FindObjectOfType<MenuManager>(); // Obtener referencia a MenuManager
    ...
    private IEnumerator PostPlayer(string nickname)
    {
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            {
                if (menuManager != null && loginUI != null)
                {
                    menuManager.CambiarPantalla(loginUI);
                }
                else
                {
                    Debug.LogError("menuManager o loginUI no están asignados.");
                }
            }

...
public class PocionController : MonoBehaviour
{
    ...
    // public GameObject pickupUI; // Eliminado
    ...
    void Update()
    {
        if (isPlayerNearby && Keyboard.current.eKey.wasPressedThisFrame && !isDropped)
        {
            if (playerId != 0)
            {   
                Debug.Log(playerId + ":" + potionId + ":" + quantity + ":" + "Recogiendo poción");
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            isPlayerNearby = true;
            // pickupUI.SetActive(true); // Eliminado
        }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            // pickupUI.SetActive(false); // Eliminado
        }
    }
    ...
    IEnumerator AddPotionToInventory(int playerId, int itemId, int quantity)
    {
        ...
        string url = "http://localhost:3003/addItem";
        PotionJsonData jsonData = new PotionJsonData(playerId, itemId, quantity);
        ...
        using (UnityWebRequest www = new UnityWebRequest(url, "POST"))
        {
            ...
            {
                Debug.Log(playerId + ":" + itemId + ":" + quantity+":"+"Ítem añadido al inventario: " + www.downloadHandler.text);
                Debug.Log(playerId + ":" + itemId + ":" + quantity);
...
[System.Serializable]
public class PotionJsonData // Cambiado el nombre de JsonData a PotionJsonData
{
    public int player_id;
    public int item_id;
    public int quantity;

    public PotionJsonData(int id, int itemID, int number)
    {
        player_id = id;
        item_id = itemID;
        quantity = number;

...
// ID del jugador en la base de datos
    ...
    void Start()
    {
        ...
        nickname = PlayerPrefs.GetString("nickname"); // Obtener el nickname del jugador
    ...
    IEnumerator UseHealthPotion()
    {
        ...
        string url = $"http://localhost:3003/inventory/{nickname}";  // URL para obtener el inventario del jugador
        UnityWebRequest request = UnityWebRequest.Get(url);
        yield return request.SendWebRequest();

        if (request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
        {
            Debug.LogError(url+ "Error al obtener el inventario: " + request.error);
    ...
    IEnumerator RemovePotionAndHeal()
    {
        ...
        JsonData1 data = new JsonData1(player_id, 2, 1);  // Aseg�rate de que JsonData est� definida correctamente
